...
def create_fp_mapping_hybrid(matches_dir, image_id_to_filename, db_features, mapping_json="fp_openmvg_to_opencv.json"):
    if os.path.exists(mapping_json):
        print(f"{mapping_json} already exists, loading mapping.")
        return load_fp_mapping_json(mapping_json)

    print("Creating feature mapping (Hybrid KDTree + brute force)...")
    idfeat_to_dbidx_map = {}
...
def extract_opencv_sift_features(sfm_data, dataset_dir):
    ...
    print(f"Extract features for {len(db_features)} images.")
...
def main():
    ...
    k_list = [30000, 32000, 34000, 36000,
