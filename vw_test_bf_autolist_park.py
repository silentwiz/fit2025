...
def run_pnp_for_query(img_path, gt_center, codebook_matcher, cluster_pts, cluster_matchers, camera_matrix, dist_coeffs, resize_ratio, err_correction):
    ...
    error = euclidean_distance(cam_pos, gt_center)*err_correction
    return error, (time.perf_counter() - start)

def main():
    root = os.getcwd()
    sfm_json = os.path.join(root, 'dataset_1', 'output', 'reconstruction_sequential', 'sfm_data.json')
    ...
    # 1) GT center 로드
    query_centers = load_query_centers(sfm_json, query_img_list)
