...
def main():
    ...
    k_num = 'k_1000' # num is clustering
   
    query_center_list = load_query_centers(sfm_json, query_img_list)
    '''
    Almost environment for solvePnPRansac,in this case 480p is recommended

    # ori_resize_ratio
    input image resolution For solvePnPRansac, 
    ::: example ::: 
    original-image-resulution(4032x 3024) -> x0.24 -> for input-image-resulution(968x726)
    
    # resize_ratio
    For camera intrinsics
    original-image-resulution(4032x 3024) -> x0.24 -> for SfM-image-resulution(968x726)
    then... resize_ratio = 0.24
    '''
    #ori_resize_ratio = 0.4762
    ori_resize_ratio = 0.24
    ori_resize_ratio = 0.66116
    #ori_resize_ratio = 0.88154
    #ori_resize_ratio = 1
    ...
    for img_rel in query_img_list:
        ...
        # cluster_matchers = vw_matchers(cluster_descs)
        # SIFT extraction
        kps, desc = extract_sift_query(img_path, ori_resize_ratio)
        # VW assignment
